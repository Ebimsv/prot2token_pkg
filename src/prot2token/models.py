...
class ProteinEncoder(nn.Module):
    def __init__(self, configs, model_name="facebook/esm2_t30_150M_UR50D", out_dim=256):
        super().__init__()
        ...
        if configs.prot2token_model.protein_encoder.lora.enable:
            ...
            not configs.prot2token_model.protein_encoder.quantization_4_bit
            and not configs.prot2token_model.protein_encoder.lora.enable
            and configs.prot2token_model.protein_encoder.fine_tune.enable
        ):
            ...
            for param in self.model.encoder.layer[
                -configs.prot2token_model.protein_encoder.fine_tune.last_layers_trainable :
            ].parameters():
                param.requires_grad = True
        ...
        self.bottleneck = nn.Conv1d(
            self.model.embeddings.position_embeddings.embedding_dim, out_dim, 1
        )
    ...
    def forward(self, x):
        features = self.model(
            input_ids=x["protein_sequence"]["input_ids"],
...
class MoleculeEncoder(nn.Module):
    def __init__(self, configs, model_name, out_dim=256):
        ...
        if configs.prot2token_model.molecule_encoder.fine_tune.enable:
            ...
            for param in self.model.encoder.layers[
                -configs.prot2token_model.molecule_encoder.fine_tune.last_layers_trainable :
            ].parameters():
                param.requires_grad = True
    ...
    def forward(self, x):
        features = self.model(
            input_ids=x["molecule_sequence"]["input_ids"],
            attention_mask=x["molecule_sequence"]["attention_mask"],
        )
        features.encoder_last_hidden_state = features.encoder_last_hidden_state.permute(
            0, 2, 1
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.positional_encoding_type = (
            self.configs.prot2token_model.positional_encoding_type
        )
        ...
        if self.positional_encoding_type == "absolute":
            self.absolute = self.pos_enc = AbsolutePositionalEmbedding(
                dim,
                max(
                    self.configs.prot2token_model.protein_encoder.max_len,
                    self.configs.prot2token_model.molecule_encoder.max_len,
                    max_len,
                ),
            )
        elif self.positional_encoding_type == "learned":
            self.decoder_pos_embed = nn.Parameter(
                torch.randn(1, max_len - 1, dim) * 0.02
            )
            self.decoder_pos_drop = nn.Dropout(p=0.05)
        else:
            raise ValueError(
                f"Unknown positional encoding type: {self.positional_encoding_type}"
            )
        ...
        if (
            self.configs.prot2token_model.protein_encoder.drop_positional_encoding
            and self.positional_encoding_type == "learned"
        ):
            self.protein_encoder_pos_embed = nn.Parameter(
                torch.randn(
                    1, self.configs.prot2token_model.protein_encoder.max_len, dim
                )
                * 0.02
            )
        ...
        if (
            self.configs.prot2token_model.molecule_encoder.drop_positional_encoding
            and self.positional_encoding_type == "learned"
            and self.configs.prot2token_model.molecule_encoder.enable
        ):
            self.molecule_encoder_pos_embed = nn.Parameter(
                torch.randn(
                    1, self.configs.prot2token_model.molecule_encoder.max_len, dim
                )
                * 0.02
            )
    ...
    def init_weights(self, logging):
        for name, p in self.named_parameters():
            if name in [
                "protein_encoder_pos_embed",
                "decoder_pos_embed",
                "molecule_encoder_pos_embed",
                "embedding.weight",
            ]:
        ...
        if (
            ...
            trunc_normal_(self.protein_encoder_pos_embed, std=0.02)
        if self.configs.prot2token_model.molecule_encoder.enable:
            if (
                self.configs.prot2token_model.molecule_encoder.drop_positional_encoding
                and self.positional_encoding_type == "learned"
            ):
                trunc_normal_(self.molecule_encoder_pos_embed, std=0.02)
    ...
    def drop_positional_encoding(self, embedding, model_type):
        ...
        if model_type == "protein":
            if (
                ...
                embedding = self.protein_encoder_pos_drop(
                    embedding + self.protein_encoder_pos_embed
                )
            elif (
                self.configs.prot2token_model.protein_encoder.drop_positional_encoding
                and self.positional_encoding_type == "absolute"
            ):
            ...
            if (
                ...
                embedding = self.molecule_encoder_pos_drop(
                    embedding + self.molecule_encoder_pos_embed
                )
            elif (
                self.configs.prot2token_model.molecule_encoder.drop_positional_encoding
                and self.positional_encoding_type == "absolute"
            ...
            if self.positional_encoding_type == "learned":
                embedding = self.decoder_pos_drop(
                    embedding + self.decoder_pos_embed[:, :seq_length, :]
                )
    ...
    def forward(self, protein_encoder_out, molecule_encoder_out, target_input):
        tgt_mask, tgt_padding_mask = create_mask(
            target_input, self.pad_idx, protein_encoder_out.device
        )
        ...
        protein_encoder_out = self.drop_positional_encoding(
            protein_encoder_out, "protein"
        )
        if self.configs.prot2token_model.molecule_encoder.enable:
            molecule_encoder_out = self.drop_positional_encoding(
                molecule_encoder_out, "molecule"
            )
    ...
    def prediction(self, protein_encoder_out, molecule_encoder_out, tgt):
        ...
        for _ in range(self.max_len - 2):
            ...
            predicted_sequence = self(
                protein_encoder_out, molecule_encoder_out, generated_tokens
            )
        ...
        padded_sequence = torch.zeros(
            ...
            predicted_sequence.shape[2],
        ).to(predicted_sequence.device)
        padded_sequence[:, :, 0] = 1
    ...
    def inference_beam_search(
        ...
        for _ in range(self.max_len - 2):
            ...
            for seq, score in beam:
                ...
                predicted_sequence = self(
                    protein_encoder_out, molecule_encoder_out, seq
                )
                ...
                for i in range(next_token_id.size(0)):
                    candidate_seq = torch.cat(
                        [seq[i : i + 1], next_token_id[i : i + 1].unsqueeze(0)], dim=-1
                    )
            ...
            if any(
                self.decoder_tokenizer.tokens_dict["<eos>"] in seq for seq, _ in beam
            ):
                break
...
class EncoderDecoder(nn.Module):
    def __init__(
        ...
        self.inference_config = {"beam_width": 3, "temperature": 1.0, "top_k": 3}
        self.device = device
        self.task_token = task_token

        self.max_protein_encoder_length = (
            configs.prot2token_model.protein_encoder.max_len
        )
        self.max_molecule_encoder_length = (
            configs.prot2token_model.molecule_encoder.max_len
        )
        self.max_decoder_length = configs.prot2token_model.decoder.max_len

        self.dummy_representation = torch.zeros(
            1, self.configs.prot2token_model.molecule_encoder.max_len, self.decoder.dim
        )
    ...
    def forward(self, batch, mode=False, **kwargs):
        ...
        if mode == "inference_greedy":
            preds = self.decoder.inference_greedy(
                protein_encoder_out, molecule_encoder_out, batch["target_input"]
            )
        elif mode == "inference_beam_search":
            preds = self.decoder.inference_beam_search(
                protein_encoder_out,
                molecule_encoder_out,
                batch["target_input"],
        ...
        for sequence in sequences:
            ...
            if self.protein_encoder_tokenizer:
                encoded_protein_sequence = self.protein_encoder_tokenizer(
                    sequence,
                    max_length=self.max_protein_encoder_length,
                    padding="max_length",
                    truncation=True,
                    return_tensors="pt",
                )

                encoded_protein_sequence["input_ids"] = encoded_protein_sequence[
                    "input_ids"
                ]
                encoded_protein_sequence["attention_mask"] = encoded_protein_sequence[
                    "attention_mask"
                ]
            else:
            ...
            if self.molecule_encoder_tokenizer:
                encoded_molecule_sequence = self.molecule_encoder_tokenizer(
                    smiles_sequence,
                ...
                encoded_molecule_sequence["input_ids"] = encoded_molecule_sequence[
                    "input_ids"
                ]
                encoded_molecule_sequence["attention_mask"] = encoded_molecule_sequence[
                    "attention_mask"
                ]
            else:
            ...
            samples_list.append(
                (
                    encoded_protein_sequence,
                    encoded_target,
                    encoded_molecule_sequence,
                    sequence,
                    task_name,
                )
            )
    ...
    def run(
        ...
        for i, sample in enumerate(
            ...
            batch = {
                "protein_sequence": protein_sequence.to(self.device),
            ...
            with torch.inference_mode():
                preds = self(
                    batch, mode=inference_type, inference_config=self.inference_config
                )
                preds = preds.detach().cpu().numpy().tolist()[0]
                preds = [
                    self.decoder_tokenizer.index_token_dict[pred]
                    for pred in preds[2:-1]
                ]
...
class InferenceTokenizer:
    ...
    def __call__(self, task_name: int):
...
def prepare_models(name, device, compile_model=False):
    ...
    molecule_encoder_tokenizer = AutoTokenizer.from_pretrained(
    ...
    configs, checkpoint_path, decoder_tokenizer_dict = prepare_config_and_checkpoint(
        name
    )
    ...
    if configs.prot2token_model.molecule_encoder.enable:
        ...
        molecule_encoder = MoleculeEncoder(
            model_name=configs.prot2token_model.molecule_encoder.model_name,
            out_dim=configs.prot2token_model.decoder.dimension,
            configs=configs,
        )
        # freeze all parameters
